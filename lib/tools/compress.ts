 ...
 */
function buildLoopEvaluationSuffix(ctx: ToolContext, rawMessages: any[]): string {
 ...
 */
export function buildAutoLoopSuffix(
    ctx: ToolContext,
    currentTokens: number,
    rawMessages?: Array<{ info: { role: string; id: string }; parts: any[] }>,
    ...
    const state: AutoLoopState = loopState ??
        ...
        const compressibleMessages = rawMessages.map((m) => ({
            ...
            isCompressed:
                m.parts?.some(
                    (p: any) =>
                        typeof p === "object" &&
                        p.text?.includes("[Compressed conversation section]"),
                ) ?? false,
        }))
    ...
    // Use state.targetTokens for consistency with buildContinuationDirective
    const stopReason =
        currentTokens <= state.targetTokens
    ...
    return tool({
        ...
        async execute(args, toolCtx) {
            ...
            try {
                ...
                const protectedToolsResult = await appendProtectedTools(
                    ctx.client,
                    ctx.state,
                    ...
                    ctx.config.compress.protectedToolRetention,
                    injected.consumedBlockIds,
                )

                const finalSummaryResult = appendMissingBlockSummaries(
                    protectedToolsResult.summary,
                ...
                const applied = applyCompressionState(
                    ...
                    protectedToolsResult.protectedContentEntries,
                )
                ...
                const postCompressionMessages = await fetchSessionMessages(
                    ctx.client,
                    toolCtx.sessionID,
                )
                const postCompressionTokens = estimateContextTokens(
                    postCompressionMessages,
                    ctx.state.systemPromptTokens ?? 0,
                )

                // Record this pass in loop state before evaluating continuation
                const preCompressionTokens = estimateContextTokens(
                    rawMessages,
                    ctx.state.systemPromptTokens ?? 0,
                )
                if (ctx.config.compress.autoLoop && ctx.state.autoLoopState) {
                ...
                const suffix = buildAutoLoopSuffix(
                    ctx,
                ...
                return (
                    `Compressed ${applied.messageIds.length} messages into ${COMPRESSED_BLOCK_HEADER}.` +
                    suffix
                )
            } catch (error) {
